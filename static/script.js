...
// Artificial delays for better UX (milliseconds)
const DELAYS = {
  nameGeneration: 2000,    // 이름 생성 시
  fortuneGeneration: 1500, // 운세 생성 시
  nameSelection: 1200      // 이름 선택 시
};
...
async function saveResultAsImage() {
  ...
  gradient.addColorStop(1, '#e8e4df');
  ctx.fillStyle = gradient;
  ctx.fillRect(0, 0, 800, 500);

  // Border
  ctx.strokeStyle = '#234B7A';
  ctx.lineWidth = 4;
  ...
  ctx.fillText(nameData.name, 400, 120);

  // Romanization
  const romanization = Array.isArray(nameData.romanization) 
    ? nameData.romanization[0] 
    : (nameData.romanization || '');
  if (romanization) {
    ctx.fillStyle = '#666';
    ctx.font = '28px sans-serif';
    ctx.fillText(romanization, 400, 165);
  }

  // Hanja
  ctx.fillStyle = '#888';
  ctx.font = '32px serif';
  ctx.fillText(nameData.hanja, 400, 210);
  ...
  wrapText(ctx, meaning, 400, 290, 600, 30);

  // Category badge
  ctx.fillStyle = '#234B7A';
  ctx.font = '18px sans-serif';
  ctx.fillText(`Category: ${nameData.category}`, 400, 380);

  // Original name
  ctx.fillStyle = '#666';
  ctx.font = '16px sans-serif';
  ctx.fillText(`Korean name for: ${currentData.originalName}`, 400, 420);

  // Branding
  ctx.fillStyle = '#999';
  ctx.font = '14px sans-serif';
  ctx.fillText('Generated by Naeilum 🌿', 400, 460);
...
function isValidName(name) {
  ...
  if (/^[^a-zA-Z가-힣ぁ-んァ-ン一-龯]+$/.test(name)) {
    return { valid: false, message: 'Name must contain letters.' };
  }
  const letterCount = (name.match(/[a-zA-Z가-힣ぁ-んァ-ン一-龯]/g) || []).length;
  if (letterCount < 2) {
...
async function handleNameSubmit(event) {
  ...
  try {
    // Artificial delay + API call
    const [data] = await Promise.all([
      postJSON('/recommend', { name, gender }),
      delay(DELAYS.nameGeneration)
    ]);
  ...
  names.forEach((nameData, index) => {
    ...
    card.innerHTML = `
      <div class="name-card-header">
        ...
        <button class="btn-icon btn-pronounce" title="Hear pronunciation" onclick="event.stopPropagation(); pronounceName('${nameData.name}')">🔊</button>
      </div>
...
function setupResultActions(nameData) {
  ...
  const buttons = [
    {
      icon: isFavorite(nameData) ? '⭐' : '☆',
      text: isFavorite(nameData) ? 'Favorited' : 'Favorite',
      action: () => {
    ...
    },
    {
      icon: '📤',
      text: 'Share',
      action: () => shareName(nameData)
    },
    {
      icon: '🔊',
      text: 'Pronounce',
      action: () => pronounceName(nameData.name)
    },
    {
      icon: '📷',
      text: 'Save Image',
      action: () => saveResultAsImage()
    }
  ];
  ...
  buttons.forEach(btn => {
    ...
    button.innerHTML = `<span class="btn-action-icon">${btn.icon}</span><span class="btn-action-text">${btn.text}</span>`;
...
function displayFortune(fortunes) {
  ...
  const fortuneStyles = {
    ...
    Family: { color: '#6C8CC9', icon: '🏠' },
    Friendship: { color: '#C27A7A', icon: '🤝' },
    Travel: { color: '#3A8570', icon: '✈️' },
    Creativity: { color: '#BD6B5F', icon: '🎨' },
    Luck: { color: '#C9A86A', icon: '🍀' }
  };
  ...
  fortunes.forEach(fortune => {
    ...
    let messageEn = fortune.message || '';
    let messageKo = fortune.message_ko || '';
